    ...
    <div className="mb-6 space-y-4">
      ...
      <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-2xl p-4 text-white shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-amber-100 text-xs mt-0.5 font-medium">
              Период: {formatDate(week.startDate)} — {formatDate(week.endDate)}
              {week.excelFileName && (
                <span className="ml-2 inline-flex items-center gap-1 text-white font-semibold">
                  • <FileSpreadsheet className="w-3.5 h-3.5" /> {week.excelFileName}
                </span>
              )}
            </p>
          </div>
        ...
        >
          ...
          <span>Вернуться к текущей неделе{currentWeekNumber ? ` (№${currentWeekNumber})` : ''}</span>
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-5 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Фиксация результатов понедельничного учета по позициям недели №{week.weekNumber}.
            </p>
          </div>
            ...
            <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200/80 text-xs text-slate-600">
              ...
              <div>
                ...
                <div className="text-[11px] text-slate-400">
                  Учет завершен {formatDateTime(closeAuditLog.createdAt)}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 mt-5">
          ...
          {/* Плитка 2: Готово */}
          <div
            onClick={() => onFilterChange(currentFilter === 'result_completed' ? 'all' : 'result_completed')}
          ...
          >
            ...
            <div className="mt-2">
              ...
              <div className="text-[11px] text-emerald-700 font-medium">позиций произведено</div>
            </div>
            <div className="text-[10px] text-emerald-600 mt-1 font-semibold">
              {currentFilter === 'result_completed' ? 'Фильтр активен' : 'Показать позиции →'}
            </div>
          </div>

          {/* Плитка 3: Забыли */}
          <div
            onClick={() => onFilterChange(currentFilter === 'result_forgotten' ? 'all' : 'result_forgotten')}
            className={`cursor-pointer rounded-xl p-4 border transition shadow-2xs flex flex-col justify-between ${
          ...
          >
            ...
            <div className="mt-2">
              <div className="text-2xl font-black text-amber-900">{stats.forgotten}</div>
              <div className="text-[11px] text-amber-700 font-medium">не запустили в цех</div>
            </div>
            <div className="text-[10px] text-amber-600 mt-1 font-semibold">
              {currentFilter === 'result_forgotten' ? 'Фильтр активен' : 'Показать позиции →'}
            </div>
          </div>

          {/* Плитка 4: Не хватило сырья */}
          <div
            onClick={() => onFilterChange(currentFilter === 'result_no_raw' ? 'all' : 'result_no_raw')}
            className={`cursor-pointer rounded-xl p-4 border transition shadow-2xs flex flex-col justify-between ${
          ...
          >
            ...
            <div className="mt-2">
              <div className="text-2xl font-black text-red-900">{stats.noRaw}</div>
              <div className="text-[11px] text-red-700 font-medium">дефицит сырья</div>
            </div>
            <div className="text-[10px] text-red-600 mt-1 font-semibold">
              {currentFilter === 'result_no_raw' ? 'Фильтр активен' : 'Показать позиции →'}
            </div>
          </div>

          {/* Плитка 5: Другое */}
          <div
            onClick={() => onFilterChange(currentFilter === 'result_other' ? 'all' : 'result_other')}
            className={`cursor-pointer rounded-xl p-4 border transition shadow-2xs flex flex-col justify-between ${
          ...
          >
            ...
            <div className="mt-2">
              <div className="text-2xl font-black text-blue-900">{stats.other}</div>
              <div className="text-[11px] text-blue-700 font-medium">с комментариями</div>
            </div>
            <div className="text-[10px] text-blue-600 mt-1 font-semibold">
              {currentFilter === 'result_other' ? 'Фильтр активен' : 'Показать позиции →'}
            </div>
          </div>
        </div>

        {/* Статус защиты архива */}
        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-end text-xs text-slate-400 font-medium">
