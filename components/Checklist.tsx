...
export default function Checklist({
   ...
   const hitsCount = items.filter(
      (i) =>
         !i.abcManualDisabled && (i.isHit || i.smartMeta?.tag === 'HIT_REPEAT')
   ).length
   const catACount = items.filter(
      (i) =>
         !i.abcManualDisabled &&
         i.abcCategory === 'A' &&
         !i.isHit &&
   ...
   const stagnantCount = items.filter(
      (i) => i.smartMeta?.tag === 'STAGNANT_STOCK'
   ).length
   const longTimeCount = items.filter(
